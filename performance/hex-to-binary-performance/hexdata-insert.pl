...
open FH , '<', 'testdata.h' or die "Cannot open testdata.h $!\n";
# there is only 1 line in the file
my $rawdata = <FH>;
my ($dummy, $hexdata);
($dummy, $dummy, $hexdata) = split /\s+/, $rawdata;
$hexdata =~ s/[";=]//g; # remove quotes
#print $hexdata;

my $sql=q{insert into hexdata (id, hexdata) values (1, ? )};
my $sth = $dbh->prepare($sql,{ora_check_sql => 0});
$sth->execute($hexdata) or die "Insert failed: " . $sth->errstr;
