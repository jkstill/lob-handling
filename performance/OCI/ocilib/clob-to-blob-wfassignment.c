...
int main(int argc, oarg* argv[])
{
    ...
    //if(argc == ARG_COUNT)
        ...
        OCI_ExecuteStmt(st, "select ida3a4, row_id from wfassignment");
        if ( DEBUG == 1) fprintf(stderr,"wfassignment query executed\n");
        ...
        while (OCI_FetchNext(rs))
        {
                ...
                OCI_Prepare(stBlob, "select ida3a4, sqldeveloper_clob_1 from wfassignment where rowid = :rowIdStr");
                ...
                if ( DEBUG == 1) fprintf(stderr,"    wfassignment query result\n");
                ...
                if (OCI_FetchNext(rsBlob))
                {
                        ...
                        OCI_Prepare(stUpd, "select 1  from wfassignment where rowid = :rowIdStr for update");
								OCI_BindString(stUpd, OTEXT(":rowIdStr"), (otext *)rowIdStr, (unsigned int)strlen(rowIdStr));
                        OCI_Execute(stUpd);

                        OCI_Prepare(stUpd, "update wfassignment set assignee = :blobData, sqldeveloper_clob_1 = empty_clob() where rowid = :rowIdStr");
								OCI_BindString(stUpd, OTEXT(":rowIdStr"), (otext *)rowIdStr, (unsigned int)strlen(rowIdStr));
