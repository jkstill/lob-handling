...
int generate_sql_templates(const char *tablename, FILE *logf) {
    ...
    snprintf(lock_buf, sizeof(lock_buf), "SELECT 1 FROM %s WHERE rowid = :rowid FOR UPDATE", tablename);
    strcat(select_buf, " FROM ");
    strcat(select_buf, tablename);
    strcat(select_buf, " WHERE rowid = :rowid");
    strcat(update_buf, " WHERE rowid = :rowid");
...
int main(void) {
    ...
    for (int i = 0; i < num_entries; ++i) {
        ...
        OCI_BindString(stSel, ":rowid", entries[i].rowid, (unsigned int)strlen(entries[i].rowid));
        if (!OCI_Execute(stSel)) continue;
        ...
        OCI_BindString(stLock, ":rowid", entries[i].rowid, (unsigned int)strlen(entries[i].rowid));
        OCI_Execute(stLock);

        OCI_Prepare(stUpd, sql_template.update_sql);
        OCI_BindString(stUpd, ":rowid", entries[i].rowid, (unsigned int)strlen(entries[i].rowid));
        for (int col = 0; col < column_count; ++col) {
